<?php
/**
 * Created by PhpStorm.
 * User: SooJu
 * Date: 2016-04-28
 * Time: 오후 5:30
 */
//SeatTable
?>
<html>
<head>
<meta charset="utf-8" />
    <title>SeatTable</title>
    <style>
        .test {
            margin: 5px;
            text-align: center;
        }
    </style>
    <script src="https://code.jquery.com/jquery-2.2.3.min.js"></script>
    <script src="js.js"></script>
</head>
<body>
<div style="width: 620px; margin: 0 auto">
    <div style="float: left; margin-right: 10px;">
        <div>
            <div class="test mouseEvent" style="float: left; background-color: greenyellow; width: 100px; height: 100px;"
                 onclick="location.href='http://localhost/aki_farm/Reserved.php?num=7'">7</div>
            <div class="test mouseEvent" style="float: left; background-color: greenyellow; width: 100px; height: 100px;"
                 onclick="location.href='http://localhost/aki_farm/Reserved.php?num=8'">8</div>
            <div class="test mouseEvent" style="float: left; background-color: greenyellow; width: 100px; height: 100px;"
                 onclick="location.href='http://localhost/aki_farm/Reserved.php?num=9'">9</div>
            <div style="clear: both;"></div>
        </div>
        <div>
            <div style="float: left;">
                <div class="test mouseEvent" style="background-color: greenyellow; width: 155px; height: 80px;"
                     onclick="location.href='http://localhost/aki_farm/Reserved.php?num=3'">3</div>
                <div class="test mouseEvent" style="background-color: greenyellow; width: 155px; height: 80px;"
                     onclick="location.href='http://localhost/aki_farm/Reserved.php?num=2'">2</div>
                <div class="test mouseEvent" style="background-color: greenyellow; width: 155px; height: 80px;"
                     onclick="location.href='http://localhost/aki_farm/Reserved.php?num=1'">1</div>
            </div>
            <div style="float: left;">
                <div class="test mouseEvent" style="background-color: greenyellow;  width: 155px; height: 80px;"
                     onclick="location.href='http://localhost/aki_farm/Reserved.php?num=6'">6</div>
                <div class="test mouseEvent" style="background-color: greenyellow;  width: 155px; height: 80px;"
                     onclick="location.href='http://localhost/aki_farm/Reserved.php?num=5'">5</div>
                <div class="test mouseEvent" style="background-color: greenyellow;  width: 155px; height: 80px;"
                     onclick="location.href='http://localhost/aki_farm/Reserved.php?num=4'">4</div>
            </div>
            <div style="clear: both"></div>
        </div>
    </div>
    <div style="float: left; ">
        <div>
            <div style="float: left;">
                <div class="test mouseEvent" style="background-color: greenyellow; width: 130px; height: 86px;"
                     onclick="location.href='http://localhost/aki_farm/Reserved.php?num=11'">11</div>
                <div class="test mouseEvent" style="background-color: greenyellow; width: 130px; height: 86px;"
                     onclick="location.href='http://localhost/aki_farm/Reserved.php?num=12'">12</div>
                <div class="test mouseEvent" style="background-color: greenyellow; width: 130px; height: 86px;"
                     onclick="location.href='http://localhost/aki_farm/Reserved.php?num=13'">13</div>
                <div class="test mouseEvent" style="background-color: greenyellow; width: 130px; height: 86px;"
                     onclick="location.href='http://localhost/aki_farm/Reserved.php?num=14'">14</div>
            </div>
            <div style="float: left;">
                <div class="test mouseEvent" style="background-color: greenyellow; width: 130px; height: 62px;"
                     onclick="location.href='http://localhost/aki_farm/Reserved.php?num=21'">21</div>
                <div class="test mouseEvent" style="background-color: greenyellow; width: 130px; height: 62px;"
                     onclick="location.href='http://localhost/aki_farm/Reserved.php?num=22'">23</div>
                <div class="test mouseEvent" style="background-color: greenyellow; width: 130px; height: 62px;"
                     onclick="location.href='http://localhost/aki_farm/Reserved.php?num=23'">25</div>
                <div class="test mouseEvent" style="background-color: greenyellow; width: 130px; height: 62px;"
                     onclick="location.href='http://localhost/aki_farm/Reserved.php?num=24'">27</div>
                <div class="test mouseEvent" style="background-color: greenyellow; width: 130px; height: 90px;"
                     onclick="location.href='http://localhost/aki_farm/Reserved.php?num=25'">30</div>
            </div>
            <div style="clear: both"></div>
        </div>
    </div>
    <div style="clear: both"></div>
</div>
</body>
</html>